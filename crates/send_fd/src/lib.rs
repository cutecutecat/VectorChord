...
use rustix::net::{
    ...
    SendAncillaryMessage, SendFlags, sockopt,
};
...
impl SendFd {
    pub fn new(timeout: Option<Duration>) -> std::io::Result<Self> {
        ...
        rx.set_read_timeout(timeout)?;
        Ok(Self {
            tx: tx.into(),
            rx: rx.into(),
        })
    }
    pub fn recv(&self, timeout: Option<Duration>) -> std::io::Result<Option<OwnedFd>> {
        let rx = self.rx.as_fd();
        sockopt::set_socket_timeout(rx, sockopt::Timeout::Recv, timeout)?;
        match recv_fd(rx) {
            Ok(fd) => Ok(Some(fd)),
            Err(e) => match rustix::io::Errno::from_io_error(&e) {
                Some(rustix::io::Errno::TIMEDOUT) => Ok(None),
                Some(rustix::io::Errno::WOULDBLOCK) => Ok(None),
                _ => Err(e),
            },
        }
    }
