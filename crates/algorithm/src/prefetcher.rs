    ...
    H::Item: Fetch + Ord,
{
    ...
    fn pop_if<'s>(
        ...
        let (e, raw) = self
            .heap
            .pop_if_until(predicate, |m| (self.filter)(m, self.with_fetch))?;
...
> Prefetcher for SimplePrefetcher<R, T, F, O>
{
    ...
    fn pop_if<'s>(
        ...
        let (e, raw) = vec_deque_pop_front_if_until(&mut self.window, predicate, |m| {
            (self.filter)(m, self.with_fetch)
        })?;
        let list = e.fetch().iter().map(|&id| self.relation.read(id)).collect();
        Some((e, list, raw))
    }
...
> Prefetcher for StreamPrefetcher<'r, R, T, F, O>
{
    ...
    fn pop_if<'s>(
        ...
        // prefilter is handled here now, as an insider prefilter will destroy stream sequence.
        // self.stream
        //     .next_if_until(predicate, |m| (self.filter)(m, self.with_fetch))
        self.stream
            .next_if(predicate)
            .map(|(e, guard)| (e, guard, None))
    }
}
...
fn vec_deque_pop_front_if_until<T, A>(
    this: &mut VecDeque<T>,
    predicate: impl FnOnce(&T) -> bool,
    mut until: impl FnMut(&T) -> (bool, A),
) -> Option<(T, A)> {
    let another = loop {
        let peek = this.front()?;
