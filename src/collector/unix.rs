...
pub fn init() {
    CHANNEL.set(SendFd::new(Some(Duration::from_secs(1))).unwrap()).ok().unwrap();
}

pub fn accept(timeout: Option<Duration>) -> Option<Socket> {
    let fd = CHANNEL.get().unwrap().recv(timeout).unwrap()?;
    pgrx::warning!("Collector accept: Received fd");
